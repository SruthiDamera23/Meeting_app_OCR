...
import { user_requests, delete_request, signup, deny_restore, signup_approve, get_church_data } from '../../../src/api'
...
const UserRequest = () => {
    ...
    const handleDeny = (userTemp) => {
        ...
        deny_restore(userTemp.id).then((req) => {
    ...
    const handleDenyNevigate = () => {
        navigate('/deny-history')
    }
    const filteredData = users.filter(item => item.deny_status === true);
                ...
                <Card className="my-card schedule-card">
                    <div className="full-screen-calendar">
                        <div style={{ display: "flex", justifyContent: "space-between", width: "100%" }}>
                            <h1 style={{ textAlign: 'left', paddingLeft: '380px', width: "100%" }}>Deny History</h1>
                        </div>
                        {isLoading ? (
                            <p>Loading...</p>
                        ) : (
                                        ...
                                        {filteredData.map((user, index) => (
                                            <tr key={index}>
                                                ...
                                                <td style={{ padding: '8px' }}>
                                                    <Button onClick={() => handleDeny(user)} color="success">Restore</Button>{' '}
                                                    <Button onClick={() => handleDelete(user)} color="danger">Delete</Button>{' '}
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
